...
Các nhóm tính năng chính: **đặt hàng đa kênh** (online/app/fanpage/tại quầy, COD/thanh toán online), **theo dõi đơn theo thời gian thực** (timeline trạng thái + mã vận đơn), **quản trị OMS/WMS/TMS** (xác nhận đơn, kiểm/trừ/hoàn kho, picking, packing, giao hàng, hủy đơn có lý do, cảnh báo tồn kho), **tìm kiếm/lọc sản phẩm** (tìm theo tên không phân biệt hoa thường/không dấu, sắp xếp theo tên/giá, gồm cả thanh tìm kiếm ở trang quản lý tồn kho), **thông báo** (chuông + badge; **mỗi đơn gộp thành 1 thông báo luôn cập nhật trạng thái mới nhất** cho khách, và admin được báo khi có đơn mới) và **quản lý tài khoản** (đổi tên/tên đăng nhập/mật khẩu, lịch sử xem hàng, lịch sử giao dịch).
...
7. Bấm **Xử lý →** vào đơn vừa đặt, thao tác lần lượt:
   ...
   - Mỗi lần chuyển trạng thái đều **gửi thông báo cho khách hàng** chủ đơn: **mỗi đơn chỉ hiện đúng 1 thông báo, được cập nhật lại nội dung + đẩy lên đầu danh sách + bật lại "chưa đọc"** theo trạng thái mới nhất (không dồn 5–6 thông báo rời rạc cho cùng một đơn).
   - **Admin cũng được báo ngay khi khách đặt đơn mới:** ngay khi checkout thành công, tất cả tài khoản admin nhận một thông báo *"Đơn hàng #… mới từ … — …đ, cần xác nhận"* (bấm vào mở thẳng trang xử lý đơn), không cần tự vào dashboard mới biết.
...
10. Bấm **icon chuông 🔔** trên đầu trang để xem **thông báo** (badge đếm số chưa đọc): **mỗi đơn hàng chỉ có một dòng thông báo, luôn cập nhật theo trạng thái mới nhất** (đơn đổi trạng thái nhiều lần cũng không sinh thêm dòng mới — dòng cũ được ghi đè nội dung và tự đánh dấu chưa đọc lại); bấm vào sẽ đánh dấu đã đọc và mở đúng đơn liên quan.
...
12. Trang **Tồn kho** (`/admin/inventory`): xem và cập nhật số lượng tồn từng sản phẩm; sản phẩm **hết hàng** (tồn = 0) tô đỏ + nhãn "Hết hàng", **sắp hết** (0 < tồn < 10) tô vàng + "Sắp hết"; có cột **Đã bán**. Cùng khối cảnh báo tồn kho như ở dashboard. Có **thanh tìm kiếm theo tên sản phẩm — không phân biệt hoa/thường và không dấu** (gõ `MAY DO` ra "Máy đo huyết áp điện tử Omron", `nuoc muoi` ra "Nước muối sinh lý…"): thanh này **chỉ lọc bảng danh sách bên dưới**, còn **khối cảnh báo tồn kho thấp phía trên vẫn hiển thị đầy đủ toàn bộ sản phẩm** liên quan, không bị lọc theo từ khóa.
